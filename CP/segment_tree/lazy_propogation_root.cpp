...
{
private:
    vector<ll> segTree; // har node pe uske segment ka SUM
    vector<ll> lazy;    // pending add jo abhi tak apply nahi hua
    int n;
    ...
    void update_Index(int i, int l, int r, int idx, int newVal)
    {
        ...
        int mid = (l + r) / 2;
        if (idx <= mid)
            update_Index(2 * i + 1, l, mid, idx, newVal);    // idx left half me
        else
            update_Index(2 * i + 2, mid + 1, r, idx, newVal); // idx right half me
...
public:
    ...
    void update_Index(int idx, int newVal)
    {
        update_Index(0, 0, n - 1, idx, newVal);
    }
