 ...
 * QUERY    : Submatrix rectangle max  [x1..x2] × [y1..y2]
 * LAZY     : NO
 * COMPLEX  : build O(n·m) | update O(log n · log m) | query O(log n · log m)
 * ════════════════════════════════════════════════════════════════════════════
 * Outer tree ROWS pe, inner tree COLUMNS pe.
 * st[vx][vy] = row-node vx ke andar column-node vy ka stored maximum.
 *
...
private:
    ...
    //        warna row internal → left+right row-child ka is column ka max lo
    //   2) column internal → mid split, left/right column subtrees build karo
    //   3) parent: st[vx][vy] = max(left col child, right col child) — MAX merge
                ...
                const vector<vector<int>> &mat)
    {
        if (sy == ey)
        {
            st[vx][vy] = (sx == ex)
                             ? mat[sx][sy]
                             : max(st[2 * vx + 1][vy], st[2 * vx + 2][vy]);
            return;
        }
        int my = (sy + ey) >> 1;
        buildY(vx, sx, ex, 2 * vy + 1, sy, my, mat);
        buildY(vx, sx, ex, 2 * vy + 2, my + 1, ey, mat);
        st[vx][vy] = max(st[vx][2 * vy + 1], st[vx][2 * vy + 2]);
    }
    ...
    // ── queryY: ek row-node ke andar columns [y1,y2] ka max ────────────────
    //   1) Case 1 NO overlap (y2 < sy || ey < y1) → IDENTITY (INT_MIN) return
    ...
    // ── queryX: rectangle [x1..x2] × [y1..y2] ka total max ─────────────────
    //   1) Case 1 NO row overlap → IDENTITY (INT_MIN) return
    //   2) Case 2 FULL row overlap → poori row range cover, andar queryY se col max
    ...
    //   1) sy == ey (column leaf):
    //        row leaf → val set | row internal → row-bachho ka col max refresh
    //   2) column internal → y <= mid se left/right child me jao
    //   3) wapas aate waqt parent column max = max(left, right)
    void updateY(int vx, int sx, int ex, int vy, int sy, int ey, int y, int val)
    {
        if (sy == ey)
        {
            st[vx][vy] = (sx == ex) ? val : max(st[2 * vx + 1][vy], st[2 * vx + 2][vy]);
            return;
        }
        int my = (sy + ey) >> 1;
        if (y <= my)
...
public:
    // ── Seg2D_Max_CompactArray: constructor ──
    //   1) n rows, m cols store karo
    //   2) st ko 4*n × 4*m allocate karo (dono dimensions safe padding)
    //   3) buildX(0, 0, n-1, mat) se root se poora 2D max tree ready karo
    Seg2D_Max_CompactArray(const vector<vector<int>> &mat)
    {
        n = (int)mat.size();
        m = (int)mat[0].size();
...
int main()
{
    ...
    cout << "---- 2D MAX | Compact Array | Point Update + Submatrix Query ----\n";
    Seg2D_Max_CompactArray st(mat);

    cout << "Whole matrix max: " << st.rectMax(0, 0, 3, 3) << endl;       // 16
    cout << "Submatrix [1,1]-[2,2]: " << st.rectMax(1, 1, 2, 2) << endl; // 11

    st.update(1, 1, 100);
    cout << "After (1,1)=100, [1,1]-[2,2]: " << st.rectMax(1, 1, 2, 2) << endl;
